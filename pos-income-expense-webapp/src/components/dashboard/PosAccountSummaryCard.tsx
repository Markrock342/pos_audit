...
export function PosAccountSummaryCard({ ledger, status }: PosAccountSummaryCardProps) {
  ...
  const startFloat = (cash?.opening ?? 0) + (cash?.deposited ?? 0);
      ...
      <Card className="border-t-4 border-t-brand">
        <CardHeader className="pb-2">
          ...
          <p className="text-xs font-normal text-text-muted">
            ทอน/ฝาก + รายรับ − รายจ่าย = เงินใน POS
          </p>
        </CardHeader>
        <CardContent className="space-y-2">
          {dayClosed ? (
            <p className="text-sm font-bold text-text-muted">
              ปิดวันแล้ว — ยอดเคลียร์เป็น 0 · ดูสรุปจริงที่ สรุปปิดยอด → ประวัติ
            </p>
          ) : cash ? (
            <>
              <Line label="เงินเริ่มต้น (ทอน + ฝาก)" value={startFloat} />
